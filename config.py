...
CSI Reader
==========
A simple Python utility to explore nexmon_csi CSI samples.
Change the config.py to match your WiFi chip and bandwidth. 
Current chip is {chip}. 
To explore a sample, type it's index from the pcap file. Indexes start from 0.
To plot a range of samples as animation, type their indexes separated by '-'.
Type 'all' to draw all samples in the pcap file.
